...
const reducer = (state: CartStateType, action: ReducerAction): CartStateType => {
  switch (action.type) {
    case REDUCER_ACTION_TYPE.ADD: {
      ...
      const { id, category, productImg, productCompany, productName, productDescription, productPrice } = action.payload;
      ...
      return {
        ...state,
        cart: [...filteredCart, { id, category, productImg, productCompany, productName, productDescription, productPrice, quantity }],
      };
    }
...
const useCartContext = (initCartState: CartStateType) => {
  ...
  const totalPrice = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(
    state.cart.reduce((previousValue, cartItem) => {
      return previousValue + cartItem.quantity * cartItem.productPrice;
    }, 0)
  );
