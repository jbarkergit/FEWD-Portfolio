...
const ProductPageDetails = ({ findProduct }: PropType) => {
  const { company, description, price, stock, unit } = findProduct;
      ...
      <div className='skuPage__grid__details__block'>
        ...
        {stock > 0 ? (
          <button
            ...
            onClick={() => dispatch({ type: REDUCER_ACTIONS.ADD, payload: { ...findProduct, quantity: 1 } })}>
            ...
            Add to Cart for {Intl.NumberFormat('en-us', { currency: 'USD', style: 'currency' }).format(price)}
          </button>
        ) : (
          <button aria-label='Out of Stock'>Out of stock</button>
        )}
      </div>
    </article>
  );
