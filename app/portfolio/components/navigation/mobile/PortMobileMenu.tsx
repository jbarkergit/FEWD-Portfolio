...
const PortMobileMenu = forwardRef<HTMLElement, Props>(
  ({ usePortMobileMenu }: Props, ref: ForwardedRef<HTMLElement>) => {
    const { setProjectSlideIndex } = usePortfolioContext();

    return (
      <section
        className='portMobileMenu'
        ref={ref}
        data-status={'false'}>
        <nav className='portMobileMenu__nav'>
          ...
          <ul
            className='portMobileMenu__nav__projects'
            aria-labelledby='project-demo-navigation'>
            {projectData.map((_, index) => (
              <li
                className='portMobileMenu__nav__projects__li'
                key={_.key + index}>
                <button
